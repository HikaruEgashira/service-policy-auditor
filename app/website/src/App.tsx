...
import { Shield, Lock, ArrowRight, Eye, AlertTriangle, Zap, Chrome } from 'lucide-react';
...
const Button = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center justify-center gap-2 rounded-full font-medium transition-colors duration-150 ${sizeClasses[size]} ${variantClasses[variant]}`}
    >
      <span>{children}</span>
      {suffix}
    </button>
    ...
    <div className="container relative z-10 mx-auto max-w-6xl px-4 text-center md:px-6">
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
          ...
          <Button variant="secondary" size="large" suffix={<ArrowRight className="h-4 w-4" />}>
            詳しく見る
          </Button>
        </div>
      </motion.div>
    ...
    <div className="container mx-auto max-w-6xl px-4 md:px-6">
      <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
        ...
        <p className="text-sm text-[#666] dark:text-[#8f8f8f]">
          Browser Security & CASB Extension
        </p>
      </div>
    </div>
  </footer>
